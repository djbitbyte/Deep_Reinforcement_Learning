...
class DDPG(object):
    def __init__(self, a_dim, s_dim, a_bound):
        ...
        with tf.variable_scope('Critic'):
            # assign self.a = a in memory when calculating a for td_error,
        ...
        # soft replacement of target net
        self.soft_replace = [[tf.assign(ta, (1 - TAU) * ta + TAU * ea), tf.assign(tc, (1 - TAU) * tc + TAU * ec)]
                             for ta, ea, tc, ec in zip(self.at_params, self.ae_params, self.ct_params, self.ce_params)]

        q_target = self.R + GAMMA * q_
        # in the feed_dict for td_error, self.a should change to actions in memory
    ...
    def _build_a(self, s, scope, trainable):
        with tf.variable_scope(scope):
            ...
            return tf.multiply(a, self.a_bound, name='scaled_a')    # normalize a
